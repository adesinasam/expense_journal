...
class ExpenseEntry(Document):
    ...
    def validate_cost_center_company(self):
        ...
        if self.default_cost_center:
            cost_center_company = frappe.db.get_value(
                "Cost Center", self.default_cost_center, "company"
            )

            if cost_center_company != self.company:
                frappe.throw(
                    _(
                        "Cost Center {0} does not belong to company {1}. It belongs to {2}."
                    ).format(
        ...
        for item in self.get("expenses", []):
            if item.expense_account:
                account_company = frappe.db.get_value(
                    "Account", item.expense_account, "company"
                )
                cost_center_company = frappe.db.get_value(
                ...
                if account_company != self.company:
                    frappe.throw(
                        _(
                            "Row #{0}: Account {1} does not belong to company {2}. It belongs to {3}."
                        ).format(
                            item.idx,
                ...
                if cost_center_company != self.company:
                    frappe.throw(
                        _(
                            "Row #{0}: Cost Center {1} does not belong to company {2}. It belongs to {3}."
                        ).format(
                            item.idx,
    ...
    def build_gl_entries(self, preview=False):
        ...
        if (
            ...
            and (not self.payment_reference or not self.clearance_date)
        ):
            frappe.throw(
